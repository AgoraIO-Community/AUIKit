...
public class AUIGiftListView: UIView, UICollectionViewDelegate, UICollectionViewDataSource,AUITabsPageContainerCellDelegate {
    ...
    lazy var flowLayout: UICollectionViewFlowLayout = {
        ...
        layout.footerReferenceSize = CGSize(width: self.frame.width, height: 150)
        return layout
    }()
...
public extension AUIGiftListView {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if kind == UICollectionView.elementKindSectionFooter {
            ...
            reusableView.frame = CGRect(origin: reusableView.frame.origin, size: CGSize(width: self.frame.width, height: 150))
            return reusableView
        }
