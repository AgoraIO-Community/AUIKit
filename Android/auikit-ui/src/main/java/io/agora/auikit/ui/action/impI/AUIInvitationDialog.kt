...
import io.agora.auikit.ui.databinding.AuiInvitationLayoutBinding
...
class AUIInvitationDialog : AUISheetFragmentDialog<AuiInvitationLayoutBinding>(), IAUIListViewBinderRefresh{
    ...
    override fun refreshInvitationData(userList:List<AUIActionUserInfo>) {
    ...
    class RoomInvitedFragmentAdapter constructor(
        ...
        init {
            this.listener = event
            with(fragments) {
                put(PAGE_INDEX0, VoiceRoomInvitedListFragment.getInstance(roomBean))
            }
        }
